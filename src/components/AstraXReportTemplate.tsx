...
import { Printer, Download, Copy, Check, Eye, Edit3, RotateCcw, Loader2 } from 'lucide-react';
import {
  renderTimeDomainChart,
  renderFFTChart,
  renderConstellationChart,
...
export const AstraXReportTemplate: React.FC<AstraXReportTemplateProps> = ({
  ...
  const formatCoordinates = (sig?: SignalProfile) => {
    if (!sig?.emitterProfile?.coordinates) return '{{coordinates}}';
    const coords = sig.emitterProfile.coordinates;
  ...
  const [fields, setFields] = useState({
    ...
    symbolRate: activeSignal
      ? (activeSignal.telemetry.symbolRateHz
          ? activeSignal.telemetry.symbolRateHz / 1e3
          : activeSignal.telemetry.symbolRateMSym * 1e3
        ).toFixed(1) + ' kSps'
      : '{{symbol_rate}}',
    ...
    coordinates: formatCoordinates(activeSignal),
    ...
    summaryText: activeSignal
      ? `Automated DSP extraction completed across ${activeSignal.durFormatted} of complex I/Q capture. Detected carrier centered with ${activeSignal.telemetry.modulation} modulation scheme at ${activeSignal.telemetry.overallConfidence.toFixed(1)}% confidence.`
  ...
  // Re-sync fields whenever activeSignal changes
  useEffect(() => {
    if (activeSignal) {
      setFields({
        docId: `NTRO-2026-A1-SIG-${activeSignal.crc32 || '7849B2'}`,
        date: new Date().toISOString().split('T')[0],
        classification: 'RESTRICTED // CONFIDENTIAL',
        fileName: activeSignal.filename,
        fileFormat: activeSignal.fileFormat,
        fileSize: activeSignal.sizeFormatted,
        duration: activeSignal.durFormatted,
        samplingRate: activeSignal.fsFormatted,
        centerFrequency: activeSignal.fcFormatted,
        modulation: activeSignal.telemetry.modulation,
        snr: `+${activeSignal.telemetry.estimatedSnrDb.toFixed(1)} dB (${activeSignal.telemetry.snrQuality})`,
        bandwidth: activeSignal.telemetry.bandwidthMHz.toFixed(3) + ' MHz',
        symbolRate: (activeSignal.telemetry.symbolRateHz
          ? activeSignal.telemetry.symbolRateHz / 1e3
          : activeSignal.telemetry.symbolRateMSym * 1e3
        ).toFixed(1) + ' kSps',
        fec: activeSignal.telemetry.fecCode,
        interleaving: activeSignal.telemetry.interleaving,
        processingTime: `${(activeSignal.processingTimeMs || 48.2).toFixed(1)} ms`,
        targetDesignation: activeSignal.emitterProfile.targetDesignation,
        callsign: activeSignal.emitterProfile.callsign,
        emitterClass: activeSignal.emitterProfile.classification,
        estimatedLocation: activeSignal.emitterProfile.estimatedLocation,
        coordinates: formatCoordinates(activeSignal),
        analysisStatus: activeSignal.isComputed ? 'COMPUTED & VERIFIED' : 'CALIBRATED',
        patternName: activeSignal.correlation.patternName,
        preambleHex: activeSignal.correlation.syncPreambleHex,
        matchedOffset: activeSignal.correlation.detectedPositionOffset,
        psr: `${activeSignal.correlation.crossCorrPsrDb.toFixed(1)} dB (${activeSignal.correlation.peakToSidelobeStatus})`,
        syncStatus: activeSignal.correlation.hasSignificantMatch ? 'LOCKED & VERIFIED' : 'SEARCHING',
        confidence: activeSignal.telemetry.overallConfidence.toFixed(1),
        summaryText: `Automated DSP extraction completed across ${activeSignal.durFormatted} of complex I/Q capture. Detected carrier centered with ${activeSignal.telemetry.modulation} modulation scheme at ${activeSignal.telemetry.overallConfidence.toFixed(1)}% confidence.`,
        findings: [
          'Carrier frequency locked with Costas loop demodulation',
          'Symbol timing synchronization converged with Gardner algorithm',
          'Preamble pattern identified with positive peak-to-sidelobe ratio',
  ...
  // Pre-render chart images when activeSignal is present
  const chartImages = useMemo(() => {
    if (!activeSignal || mode === 'template') return null;
    return {
      timeDomain: renderTimeDomainChart(activeSignal, 480, 160),
      fft: renderFFTChart(activeSignal, 480, 160),
      constellation: renderConstellationChart(activeSignal, 240, 240),
      waterfall: renderWaterfallChart(activeSignal, 480, 160),
    };
  }, [activeSignal, mode]);
  ...
  const handleExportPdf = async () => {
    ...
    try {
      setIsExporting(true);
      const { generateSignalReportPdf, getReportFilename } = await import('../lib/pdfGenerator');
      const blob = await generateSignalReportPdf(activeSignal);
...
`;
  ...
  // The Master A4 Sheet JSX
  const renderSheet = () => (
    <div
      ref={reportRef}
      id="astrax-a4-report-sheet"
          ...
          <div className="col-span-6 flex flex-col gap-2.5">
            ...
            <div>
              ...
              <div className="h-20 border border-[#D6DDE5] rounded-sm relative bg-white flex flex-col justify-between p-1">
                ...
                <div className="ml-7 mr-2 h-16 border border-[#E2E8F0] relative overflow-hidden bg-white">
                  {isTemplate && (
                    <div className="absolute inset-0 flex items-center justify-center font-mono text-sm text-slate-400 z-20">
                      {'{{chart_2}}'}
                    </div>
                  )}
                  ...
                  {!isTemplate && chartImages?.timeDomain && (
                    <img
                      src={chartImages.timeDomain}
                      alt="Time Domain Waveform"
            ...
            <div>
              ...
              <div className="h-20 border border-[#D6DDE5] rounded-sm relative bg-white flex flex-col justify-between p-1">
                ...
                {/* Empty Grid Interior or Populated FFT */}
                <div className="ml-7 mr-2 h-16 border border-[#E2E8F0] relative overflow-hidden bg-white">
                  {isTemplate && (
                    <div className="absolute inset-0 flex items-center justify-center font-mono text-sm text-slate-400 z-20">
                      {'{{chart_3}}'}
            ...
            <div>
              ...
              <div className="grid grid-cols-12 gap-2">
                ...
                <div className="col-span-7 h-24 border border-[#D6DDE5] rounded-sm relative bg-white flex items-center justify-center overflow-hidden">
                  {isTemplate && (
                    <div className="absolute inset-0 flex items-center justify-center font-mono text-sm text-slate-400 z-20 pointer-events-none">
                      {'{{chart_5}}'}
                    </div>
                  )}

                  {/* Y-axis (Q) ticks */}
                  <div className="absolute left-1 top-0 bottom-1 flex flex-col justify-between text-[7px] font-mono text-slate-500 z-10">
                    <span>2</span>
                    <span>1</span>
                    <span>0</span>
                    <span>-1</span>
                  ...
                  {/* Populated constellation image */}
                  {!isTemplate && chartImages?.constellation && (
                    <img
                      src={chartImages.constellation}
            ...
            <div>
              ...
              <div className="h-20 border border-[#D6DDE5] rounded-sm relative bg-white flex items-center p-1">
                ...
                <div className="ml-6 mr-14 h-16 w-full border border-[#E2E8F0] relative overflow-hidden bg-white">
                  ...
                  {!isTemplate && chartImages?.waterfall && (
                    <img
                      src={chartImages.waterfall}
          ...
          <div className="h-10 border border-[#D6DDE5] rounded-sm bg-white px-2.5 py-1 font-mono text-[9px] text-[#4B6584] flex items-center">
            {isTemplate ? (
              ...
              <span className="text-[#082A4A] font-semibold truncate">
                {activeSignal?.bitstreamLines && activeSignal.bitstreamLines.length > 0
                  ? `${activeSignal.bitstreamLines[0].offset}: ${activeSignal.bitstreamLines[0].hexBytes.join(' ')}  [${activeSignal.bitstreamLines[0].ascii}]`
  ...
  // If hideControls is specified (e.g. during offscreen PDF generation), render only the A4 sheet
  if (hideControls) {
    return renderSheet();
  }
      ...
      <div className="w-full max-w-[210mm] mb-5 flex flex-wrap items-center justify-between gap-3 bg-[#171b26] p-3.5 rounded-xl border border-[#262a35] text-slate-200 print:hidden shadow-lg">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 bg-[#262a35] hover:bg-[#353944] text-slate-200 font-mono text-xs rounded border border-[#3d494c] transition cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 bg-[#6D28D9] hover:bg-[#8B5CF6] text-white font-mono text-xs font-bold rounded shadow-md transition disabled:opacity-50 cursor-pointer"
            title="Export official AstraX NTRO A4 PDF"
          >
            {isExporting ? (
              <>
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1677D2] hover:bg-[#125ba3] text-white font-mono text-xs font-bold rounded shadow-md transition cursor-pointer"
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="px-3 py-1.5 bg-[#262a35] hover:bg-rose-950/60 hover:text-rose-300 text-slate-300 font-mono text-xs rounded border border-[#313540] transition cursor-pointer"
            >
              Close
            </button>
          )}
